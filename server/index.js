...
app.post('/api/generate-lesson', async (req, res) => {
    ...
    ${userRequest ? 'The user has made a direct request, so provide a specific tutorial for this task.' : 'Based on the Blender 5.0 Manual, provide a 3-step interactive lesson script.'}
    ...
    Return ONLY the JSON array.`;
        ...
        if (!resp.response.candidates || resp.response.candidates.length === 0) {
            return res.status(500).json({
                success: false,
                error: 'No candidates returned. Safety filters might have blocked the response.',
